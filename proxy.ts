...
export function proxy(request:NextRequest){
 ...
 if(!request.cookies.has("go_access_token")&&!request.cookies.has("go_refresh_token")){const login=new URL("/login",request.url);login.searchParams.set("returnTo",pathname);return NextResponse.redirect(login)}
