...
const configs: MasterConfig[] = [
  ...
  },
  {
    ...
    columns: [{ label: "الرمز", key: "code" }, { label: "الباقة", key: "name" }, { label: "العقد", key: "contractDisplay" }, { label: "النوع", key: "fulfillmentKind" }, { label: "المدة", key: "durationDays" }, { label: "حالة النشر", key: "status" }],
    ...
    initial: { durationValue: "1", durationUnit: "MONTHS", accessFrequency: "UNLIMITED", branchAccessPolicy: "SINGLE_BRANCH", fulfillmentKind: "FACILITY_ACCESS", contractType: "NONE" },
  ...
  },
  {
    ...
    columns: [{ label: "الباقة أو الخدمة", key: "targetDisplay" }, { label: "النوع", key: "targetType" }, { label: "القيمة", key: "amountMinor" }, { label: "النطاق", key: "branchId" }, { label: "الضريبة", key: "taxRateBps" }, { label: "ساري من", key: "validFrom" }, { label: "الحالة", key: "status" }],
  ...
  {
    id: "promotions", label: "العروض والخصومات", description: "إنشاء عروض مستهدفة، أو كود خصم عام يعمل مع جميع الباقات.", permission: "commercial.read", managePermission: "promotions.manage", path: "/organizations/{organizationId}/promotions", createPath: "/organizations/{organizationId}/promotions", updatePath: id => `/organizations/{organizationId}/promotions/${id}`,
    columns: [{ label: "الكود", key: "code" }, { label: "العرض", key: "name" }, { label: "الفائدة", key: "benefitType" }, { label: "القيمة", key: "benefitValue" }, { label: "الأهلية", key: "eligibility" }, { label: "النطاق", key: "promotionScope" }, { label: "الفروع", key: "branchIds" }, { label: "ينتهي", key: "validUntil" }, { label: "الحالة", key: "status" }],
    createFields: [{ name: "code", label: "كود العرض", required: true, hint: "يمكن كتابته بأحرف إنجليزية وأرقام، مثل SUMMER25. سيحوّله النظام إلى أحرف كبيرة تلقائيًا." }, { name: "name", label: "اسم العرض", required: true }, { name: "benefitType", label: "نوع الفائدة", type: "select", required: true, options: [{ value: "PERCENTAGE", label: "خصم بالنسبة المئوية" }, { value: "FIXED_DISCOUNT", label: "خصم مبلغ ثابت" }, { value: "FIXED_FINAL_PRICE", label: "سعر نهائي ثابت" }] }, { name: "benefitValue", label: "قيمة العرض", type: "number", required: true, hint: "للنسبة أدخل مثلًا 25 لخصم 25٪، وللمبلغ أدخل القيمة بالريال." }, { name: "eligibility", label: "طريقة تطبيق العرض", type: "select", required: true, options: [{ value: "EVERYONE", label: "تلقائي لجميع الأعضاء المؤهلين" }, { value: "NEW_MEMBER", label: "تلقائي للأعضاء الجدد" }, { value: "FORMER_MEMBER", label: "تلقائي للأعضاء السابقين" }, { value: "PROMO_CODE", label: "يدوي عند إدخال كود الخصم" }] }, { name: "targetScope", label: "نطاق كود الخصم", type: "select", required: true, visibleWhen: { field: "eligibility", values: ["PROMO_CODE"] }, options: [{ value: "ALL_PACKAGES", label: "جميع الباقات — كود عام" }, { value: "TARGETED", label: "باقات أو خدمات محددة" }], hint: "الكود العام يسري على جميع الباقات الحالية والمستقبلية، مع احترام الفروع وفترة الصلاحية." }, { name: "validFrom", label: "يبدأ العرض", type: "datetime-local", required: true }, { name: "validUntil", label: "ينتهي العرض", type: "datetime-local", required: true }, { name: "branchIds", label: "الفروع المستهدفة", type: "multi", source: "branches", hint: "اتركها دون تحديد ليكون العرض متاحًا في كل الفروع." }, { name: "packageIds", label: "الباقات المشمولة", type: "multi", source: "packages" }, { name: "serviceIds", label: "الخدمات المشمولة", type: "multi", source: "services" }],
    initial: { benefitType: "PERCENTAGE", eligibility: "PROMO_CODE", targetScope: "ALL_PACKAGES", validFrom: dateTimeLocal(new Date()), validUntil: dateTimeLocal(new Date(Date.now() + 30 * 86_400_000)) },
    createBody: values => ({ code: String(values.code).trim().toUpperCase(), name: values.name, benefitType: values.benefitType, benefitValue: Math.round(number(values.benefitValue) * 100), eligibility: values.eligibility, validFrom: new Date(String(values.validFrom)).toISOString(), validUntil: new Date(String(values.validUntil)).toISOString(), branchIds: asArray(values.branchIds), targets: promotionTargets(values) }),
    editFields: [{ name: "name", label: "اسم العرض", required: true }, { name: "benefitType", label: "نوع الفائدة", type: "select", required: true, options: [{ value: "PERCENTAGE", label: "خصم بالنسبة المئوية" }, { value: "FIXED_DISCOUNT", label: "خصم مبلغ ثابت" }, { value: "FIXED_FINAL_PRICE", label: "سعر نهائي ثابت" }] }, { name: "benefitValue", label: "قيمة العرض", type: "number", required: true }, { name: "eligibility", label: "طريقة تطبيق العرض", type: "select", required: true, options: [{ value: "EVERYONE", label: "تلقائي لجميع الأعضاء المؤهلين" }, { value: "NEW_MEMBER", label: "تلقائي للأعضاء الجدد" }, { value: "FORMER_MEMBER", label: "تلقائي للأعضاء السابقين" }, { value: "PROMO_CODE", label: "يدوي عند إدخال كود الخصم" }] }, { name: "targetScope", label: "نطاق كود الخصم", type: "select", required: true, visibleWhen: { field: "eligibility", values: ["PROMO_CODE"] }, options: [{ value: "ALL_PACKAGES", label: "جميع الباقات — كود عام" }, { value: "TARGETED", label: "باقات أو خدمات محددة" }], hint: "الكود العام يسري على جميع الباقات الحالية والمستقبلية، مع احترام الفروع وفترة الصلاحية." }, { name: "validFrom", label: "يبدأ العرض", type: "datetime-local", required: true }, { name: "validUntil", label: "ينتهي العرض", type: "datetime-local", required: true }, { name: "branchIds", label: "الفروع المستهدفة", type: "multi", source: "branches" }, { name: "packageIds", label: "الباقات المشمولة", type: "multi", source: "packages" }, { name: "serviceIds", label: "الخدمات المشمولة", type: "multi", source: "services" }, { name: "status", label: "الحالة", type: "select", required: true, options: [{ value: "ACTIVE", label: "نشط" }, { value: "INACTIVE", label: "محذوف / مؤرشف" }] }],
    editBody: (values, item) => ({ name: values.name, benefitType: values.benefitType, benefitValue: Math.round(number(values.benefitValue) * 100), eligibility: values.eligibility, validFrom: new Date(String(values.validFrom)).toISOString(), validUntil: new Date(String(values.validUntil)).toISOString(), branchIds: asArray(values.branchIds), targets: promotionTargets(values), status: values.status, expectedVersion: Number(item.version ?? 1) }),
  },
  {
    ...
    editFields: [{ name: "name", label: "اسم واضح للسياسة", required: true }, { name: "maxDaysPerFreeze", label: "أقصى أيام للتجميد", type: "number", visibleWhen: { field: "policyType", values: ["FREEZE"] } }, { name: "maxFreezesPerTerm", label: "أقصى مرات تجميد للاشتراك", type: "number", visibleWhen: { field: "policyType", values: ["FREEZE"] } }, { name: "minimumActiveDaysBeforeFreeze", label: "الحد الأدنى لأيام سريان الاشتراك قبل التجميد", type: "number", visibleWhen: { field: "policyType", values: ["FREEZE"] }, hint: "عدد الأيام التي يجب أن يظل فيها الاشتراك ساريًا قبل السماح بتجميده، وليس عدد أيام الحضور." }, { name: "cancellationMode", label: "موعد الإلغاء", type: "select", visibleWhen: { field: "policyType", values: ["CANCELLATION"] }, options: [{ value: "END_OF_TERM", label: "نهاية فترة الاشتراك" }, { value: "IMMEDIATE_PRORATED", label: "فوري مع استرداد نسبي" }] }, { name: "noticeDays", label: "فترة الإشعار بالأيام", type: "number", visibleWhen: { field: "policyType", values: ["CANCELLATION"] } }, { name: "fee", label: "رسوم الإلغاء بالريال", type: "number", visibleWhen: { field: "policyType", values: ["CANCELLATION"] } }, { name: "graceDays", label: "مهلة التجديد بعد انتهاء الاشتراك (أيام)", type: "number", visibleWhen: { field: "policyType", values: ["RENEWAL"] }, hint: "التجديد متاح طوال الاشتراك النشط، وتحدد هذه القيمة المهلة بعد انتهائه فقط." }, { name: "cutoffHours", label: "آخر موعد للإلغاء قبل الحجز (ساعات)", type: "number", visibleWhen: { field: "policyType", values: ["BOOKING_CANCELLATION"] } }, { name: "refundPercentage", label: "نسبة الاسترداد %", type: "number", visibleWhen: { field: "policyType", values: ["BOOKING_CANCELLATION"] } }],
...
function isHistoricalPackage(item: RecordItem) {
  return String(item.code ?? "").trim().toUpperCase().startsWith("LEGACY_") || String(item.name ?? "").trim().startsWith("[تاريخي]")
}
function sortMasterItems(configId: string, values: RecordItem[]) {
  if (configId !== "packages") return values
  return [...values].sort((left, right) => {
    const historyOrder = Number(isHistoricalPackage(left)) - Number(isHistoricalPackage(right))
...
function itemText(item: RecordItem, key: string, branches: BranchLookup[] = []) {
  if (key === "promotionScope") return Array.isArray(item.targets) && item.targets.length === 0 ? "جميع الباقات" : "نطاق محدد"
  if (key === "contractDisplay") { const contract = item.contract && typeof item.contract === "object" ? item.contract as RecordItem : undefined; return contract ? `${contract.type === "CHILD_ACADEMY" ? "طفل / ولي أمر" : "مشترك بالغ"} — ${String(contract.title ?? "عقد")}` : "بدون عقد" }
  if (key === "targetDisplay") { const name = String(item.targetName ?? ""); const code = String(item.targetCode ?? ""); if (name) return code && code !== name ? `${name} (${code})` : name; return item.targetType === "SERVICE" ? "خدمة غير متاحة" : "باقة غير متاحة" }
  if (key === "targetType") return item.targetType === "SERVICE" ? "خدمة" : item.targetType === "PACKAGE" ? "باقة" : text(item[key], key)
  if (key === "contractType") return item[key] === "CHILD_ACADEMY" ? "أكاديمية أطفال" : "نشاط عام"
  if (key === "amountMinor" && item.amount && typeof item.amount === "object") return money((item.amount as RecordItem).minorUnits)
  if (key === "benefitValue" && item.benefitType === "PERCENTAGE") return `${Number(item[key]) / 100}%`
  if (key === "branchId") { const branch = branches.find(candidate => candidate.id === item[key]); return branch?.nameAr ?? branch?.name ?? (item[key] ? "فرع محدد" : "جميع الفروع") }
  if (key === "hasLoginAccount") return item[key] ? "مفعّل" : "غير مُنشأ"
...
function formValueFrom(configId: string, item: RecordItem, key: string): Value {
  ...
  if (configId === "promotions" && key === "targetScope") return Array.isArray(item.targets) && item.targets.length === 0 ? "ALL_PACKAGES" : "TARGETED"
  ...
  if (configId === "packages" && ["contractType", "contractTitle", "contractContent"].includes(key)) {
    const contract = item.contract && typeof item.contract === "object" ? item.contract as RecordItem : undefined
    if (key === "contractType") return String(contract?.type ?? "NONE")
    return String(contract?.[key === "contractTitle" ? "title" : "content"] ?? "")
  }
...
function archivePayload(config: MasterConfig, item: RecordItem): Record<string, unknown> {
  ...
  if (config.editBody) { const body = config.editBody(values, item); if (config.id === "packages") body.contract = item.contract ?? null; return body }
...
export function MasterDataPage({ initialSection = "branches" }: { initialSection?: string }) {
  ...
  const filtered = sortMasterItems(active.id, items.filter(item => Object.values(item).some(value => String(value ?? "").toLowerCase().includes(query.toLowerCase()))))
  ...
  return <div className="fade-up" dir="rtl">
    ...
    <div className="grid gap-5 lg:grid-cols-[250px_minmax(0,1fr)]"><Card className="h-fit"><CardContent className="p-2"><p className="px-3 pb-2 pt-3 text-[10px] font-bold text-muted-foreground">مجموعات الإدارة</p><div className="space-y-4">{navigationGroups.map(group => { const groupConfigs = group.ids.map(id => allowed.find(config => config.id === id)).filter((config): config is MasterConfig => Boolean(config)); return groupConfigs.length ? <section key={group.label}><p className="px-3 pb-1 text-[10px] font-bold text-muted-foreground">{group.label}</p><div className="space-y-1">{groupConfigs.map(config => <Link key={config.id} href={`/system-settings/${config.id}`} onClick={() => { setActiveId(config.id); setQuery("") }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-right text-xs font-bold transition ${active.id === config.id ? "bg-primary text-primary-foreground" : "hover:bg-secondary"}`}><Settings2 className="size-4" />{config.label}<ChevronLeft className="mr-auto size-3 opacity-60" /></Link>)}</div></section> : null })}</div></CardContent></Card>
      <Card className="overflow-hidden"><div className="flex flex-col gap-4 border-b p-5 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="grid size-9 place-items-center rounded-xl bg-primary/15 text-amber-700"><Building2 className="size-4" /></span><div><h2 className="font-black">{active.label}</h2><p className="mt-0.5 text-[11px] text-muted-foreground">{active.description}</p></div></div></div>{active.createFields && canManageActive && <Button onClick={() => setForm({ mode: "create" })}><Plus />{createLabel}</Button>}</div>{exceptionalPermissions && <div className="flex items-start gap-3 border-b border-amber-500/20 bg-amber-500/10 px-5 py-4 text-xs leading-6"><ShieldAlert className="mt-0.5 size-5 shrink-0 text-amber-700" /><div><p className="font-black">{active.id === "role-assignments" ? "هذه ليست قائمة كل الموظفين" : "هذه المجموعات ليست بديلًا عن المسمى الوظيفي"}</p><p className="text-muted-foreground">{active.id === "role-assignments" ? "يعرض الجدول فقط الموظفين الذين لديهم صلاحيات إضافية بالفعل. لإضافة موظف آخر اضغط «إضافة صلاحيات لموظف»؛ وستظهر جميع حسابات الموظفين في القائمة." : "الموظف يحصل تلقائيًا على صلاحيات مسماه الوظيفي وفرعه. أنشئ هنا فقط مجموعة إضافية لحاجة إدارية خاصة أو لإدارة أكثر من فرع."}</p></div></div>}{active.branchScoped && <div className="flex items-center gap-2 border-b bg-amber-500/8 px-5 py-3 text-xs"><Building2 className="size-4 text-amber-700" /><span className="font-bold">نطاق هذه البيانات:</span><span>{selectedBranch?.nameAr ?? selectedBranch?.name ?? "الفرع الحالي"}</span><span className="text-muted-foreground">— غيّر الفرع من الشريط العلوي لعرض أو إدارة فرع آخر.</span></div>}{active.id === "packages" && <div className="flex items-center gap-2 border-b border-amber-500/20 bg-amber-500/[0.06] px-5 py-3 text-xs"><Badge variant="outline" className="border-amber-500/40 bg-amber-500/15 text-amber-700">باقة تاريخية</Badge><span className="text-muted-foreground">الباقات الجديدة تظهر أولًا، والباقات التاريخية المميزة محفوظة لبيانات الاشتراكات القديمة.</span></div>}<div className="border-b p-4"><div className="relative max-w-md"><Search className="absolute right-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={event => setQuery(event.target.value)} className="pr-10" placeholder={`ابحث في ${active.label}...`} /></div></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-right text-xs"><thead className="bg-secondary/45"><tr>{active.columns.map(column => <th key={column.key} className="px-5 py-3 text-[10px] font-bold text-muted-foreground">{column.label}</th>)}<th className="w-20 px-5" /></tr></thead><tbody className="divide-y">{filtered.map(item => {
          const canArchive = Boolean(active.archivePath || (active.updatePath && active.editFields?.some(field => field.name === "status"))) && item.status !== "INACTIVE" && item.status !== "REVOKED"
          const historicalPackage = active.id === "packages" && isHistoricalPackage(item)
          return <tr key={itemId(item)} className={historicalPackage ? "bg-amber-500/[0.07] transition-colors hover:bg-amber-500/[0.12]" : "transition-colors hover:bg-secondary/30"}>{active.columns.map(column => <td key={column.key} className="px-5 py-4">{column.key === "status" ? <StatusBadge status={String(item[column.key] ?? "—")} /> : historicalPackage && column.key === "name" ? <div className="flex items-center gap-2"><span className="max-w-64 truncate font-medium">{itemText(item, column.key, context.branches)}</span><Badge variant="outline" className="shrink-0 border-amber-500/40 bg-amber-500/15 text-[10px] text-amber-700">تاريخية</Badge></div> : <span className="max-w-64 truncate font-medium">{itemText(item, column.key, context.branches)}</span>}</td>)}<td className="px-5"><div className="flex items-center gap-1">{active.id === "packages" && item.status === "DRAFT" && canManageActive && <Button variant="ghost" size="sm" disabled={publishingPackageId === itemId(item)} onClick={() => void publishPackage(item)}>{publishingPackageId === itemId(item) ? <Loader2 className="animate-spin" /> : "نشر"}</Button>}{active.id === "services" && context.canAccess(["catalog.availability.manage"]) && <Button variant="ghost" size="sm" onClick={() => setAvailabilityService(item)}>تفعيل بالفرع</Button>}{active.id === "bookable-resources" && canManageActive && <Button variant="ghost" size="sm" onClick={() => setAvailabilityResource(item)}>الإتاحة</Button>}{active.updatePath && active.editFields && canManageActive && <Button variant="ghost" size="sm" onClick={() => setForm({ mode: "edit", item })}><Pencil />تعديل</Button>}{canArchive && canManageActive && <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" disabled={archivingId === itemId(item)} onClick={() => void archiveItem(item)}>{archivingId === itemId(item) ? <Loader2 className="animate-spin" /> : <Trash2 />}حذف</Button>}</div></td></tr>
        })}</tbody></table>{loading ? <div className="grid place-items-center p-14"><Loader2 className="size-7 animate-spin text-primary" /></div> : error ? <div className="p-10 text-center"><p className="font-bold text-destructive">تعذر عرض البيانات</p><p className="mt-2 text-xs text-muted-foreground">{error}</p><Button className="mt-4" variant="outline" onClick={() => void load()}>إعادة المحاولة</Button></div> : !filtered.length && <div className="p-14 text-center text-sm text-muted-foreground">لا توجد سجلات مطابقة.</div>}</div></Card></div>
...
function ResourceAvailabilityDialog({ resource, organizationId, branchId, onClose }: { resource: RecordItem; organizationId: string; branchId: string; onClose: () => void }) {
  ...
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState("")
  const [loadingSlots, setLoadingSlots] = useState(usesSessionSlots)
  const [savingSlot, setSavingSlot] = useState(false)
  const [slotError, setSlotError] = useState("")
  const [values, setValues] = useState({ dayOfWeek: "0", startLocal: "08:00", endLocal: "22:00", validFrom: dateTimeLocal(new Date()).slice(0, 10), validUntil: "" })
  ...
  const loadSlots = useCallback(async () => {
    if (!resourceId || !usesSessionSlots) return
    setLoadingSlots(true)
    const from = new Date(); const to = new Date(from.getTime() + 30 * 86_400_000)
    try { const response = await apiRequest<unknown>(`${slotsPath}?from=${encodeURIComponent(from.toISOString())}&to=${encodeURIComponent(to.toISOString())}`); setSlots(listOf(response.data)); setSlotError("") }
    catch (reason) { setSlotError(humanError(reason, "تعذر تحميل مواعيد المورد.")) }
    finally { setLoadingSlots(false) }
  }, [resourceId, slotsPath, usesSessionSlots])
  useEffect(() => {
    const frame = requestAnimationFrame(() => { void loadRules(); void loadSlots() })
    if (resourceType === "PERSONAL_TRAINING") void apiRequest<unknown>(`/organizations/${organizationId}/trainers?branchId=${encodeURIComponent(branchId)}&limit=100`).then(response => setTrainers(listOf(response.data).filter(item => item.status === "ACTIVE"))).catch(reason => setSlotError(humanError(reason, "تعذر تحميل قائمة المدربين المتاحين في الفرع.")))
    return () => cancelAnimationFrame(frame)
  }, [branchId, loadRules, loadSlots, organizationId, resourceType])
  async function submit(event: React.FormEvent) {
    event.preventDefault(); setSaving(true); setError("")
    try {
  ...
  async function submitSlot(event: React.FormEvent) {
    ...
    const startsAt = new Date(slotValues.startsAt); const endsAt = new Date(slotValues.endsAt); const capacity = Number(slotValues.capacity)
    if (Number.isNaN(startsAt.getTime()) || Number.isNaN(endsAt.getTime()) || endsAt <= startsAt) { setSlotError("نهاية الموعد يجب أن تكون بعد بدايته."); return }
    if (startsAt <= new Date()) { setSlotError("اختر موعدًا في المستقبل."); return }
    if (!Number.isInteger(capacity) || capacity < 1 || capacity > Number(resource.capacity ?? 1)) { setSlotError(`السعة يجب أن تكون بين 1 و${String(resource.capacity ?? 1)}.`); return }
    if (resourceType === "PERSONAL_TRAINING" && !slotValues.trainerProfileId) { setSlotError("اختر المدرب المسؤول عن موعد التدريب الشخصي."); return }
    setSavingSlot(true)
    try {
      await apiRequest(slotsPath, { method: "POST", idempotencyKey: createIdempotencyKey(), body: JSON.stringify({ branchId, startsAt: startsAt.toISOString(), endsAt: endsAt.toISOString(), capacity, ...(resourceType === "PERSONAL_TRAINING" ? { trainerProfileId: slotValues.trainerProfileId } : {}) }) })
      await loadSlots()
    } catch (reason) { setSlotError(humanError(reason, "تعذر إنشاء موعد الحجز.")) }
    finally { setSavingSlot(false) }
  }

  return <div className="fixed inset-0 z-[85] grid place-items-end bg-black/60 backdrop-blur-sm sm:place-items-center sm:p-5" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}><section role="dialog" aria-modal="true" aria-labelledby="availability-title" className="max-h-[94vh] w-full overflow-y-auto rounded-t-[28px] border bg-card shadow-2xl sm:max-w-3xl sm:rounded-[28px]"><header className="sticky top-0 z-10 flex items-start border-b bg-card/95 p-5 backdrop-blur"><div><p className="text-[11px] font-bold text-amber-700">موارد الحجز</p><h2 id="availability-title" className="mt-1 text-xl font-black">إتاحة ومواعيد {String(resource.name ?? resource.code ?? "المورد")}</h2><p className="mt-1 text-[11px] text-muted-foreground">حدد ساعات العمل الأسبوعية أولًا، ثم أضف المواعيد الفعلية للحصص أو التدريب.</p></div><Button className="mr-auto" variant="ghost" size="icon" onClick={onClose} aria-label="إغلاق"><X /></Button></header><div className="space-y-7 p-5 sm:p-6"><section><h3 className="mb-3 text-sm font-black">ساعات الإتاحة الأسبوعية</h3><form onSubmit={submit} className="grid gap-4 rounded-2xl border bg-secondary/25 p-4 sm:grid-cols-2"><label className="text-xs font-bold">اليوم<select value={values.dayOfWeek} onChange={event => setValues(current => ({ ...current, dayOfWeek: event.target.value }))} className="mt-2 h-11 w-full rounded-xl border bg-background px-3 text-sm">{weekdayNames.map((name, index) => <option key={name} value={index}>{name}</option>)}</select></label><label className="text-xs font-bold">يبدأ من<DateTimeInput type="time" value={values.startLocal} onChange={event => setValues(current => ({ ...current, startLocal: event.target.value }))} className="mt-2 h-11" /></label><label className="text-xs font-bold">ينتهي في<DateTimeInput type="time" value={values.endLocal} onChange={event => setValues(current => ({ ...current, endLocal: event.target.value }))} className="mt-2 h-11" /></label><label className="text-xs font-bold">سارية من<DateTimeInput type="date" value={values.validFrom} onChange={event => setValues(current => ({ ...current, validFrom: event.target.value }))} className="mt-2 h-11" /></label><label className="text-xs font-bold">تنتهي في (اختياري)<DateTimeInput type="date" value={values.validUntil} onChange={event => setValues(current => ({ ...current, validUntil: event.target.value }))} className="mt-2 h-11" /></label><div className="flex items-end"><Button type="submit" className="w-full" disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <Check />}إضافة وقت الإتاحة</Button></div></form>{error && <p role="alert" className="mt-4 rounded-xl bg-destructive/10 p-3 text-xs font-bold text-destructive">{error}</p>}<div className="mt-4">{loading ? <div className="grid place-items-center p-8"><Loader2 className="animate-spin text-primary" /></div> : rules.length ? <div className="grid gap-2 sm:grid-cols-2">{rules.map(rule => <div key={itemId(rule)} className="rounded-xl border p-3"><p className="text-xs font-bold">{weekdayNames[Number(rule.dayOfWeek)] ?? "يوم غير محدد"}</p><p className="mt-1 text-[11px] text-muted-foreground">{String(rule.startLocal)} — {String(rule.endLocal)}</p><p className="mt-1 text-[10px] text-muted-foreground">من {String(rule.validFrom)}{rule.validUntil ? ` إلى ${String(rule.validUntil)}` : ""}</p></div>)}</div> : <p className="rounded-xl border border-dashed p-5 text-center text-xs text-muted-foreground">لا توجد أوقات إتاحة بعد.</p>}</div></section>
    {usesSessionSlots && <section className="border-t pt-6"><h3 className="text-sm font-black">مواعيد الحجز الفعلية</h3><p className="mt-1 text-[11px] text-muted-foreground">يجب أن يقع الموعد داخل إحدى ساعات الإتاحة أعلاه.</p><form onSubmit={submitSlot} className="mt-3 grid gap-4 rounded-2xl border bg-secondary/25 p-4 sm:grid-cols-2"><label className="text-xs font-bold">بداية الموعد<DateTimeInput required type="datetime-local" value={slotValues.startsAt} onChange={event => setSlotValues(current => ({ ...current, startsAt: event.target.value }))} className="mt-2 h-11" /></label><label className="text-xs font-bold">نهاية الموعد<DateTimeInput required type="datetime-local" value={slotValues.endsAt} onChange={event => setSlotValues(current => ({ ...current, endsAt: event.target.value }))} className="mt-2 h-11" /></label><label className="text-xs font-bold">سعة الموعد<Input required type="number" min="1" max={Number(resource.capacity ?? 1)} value={slotValues.capacity} disabled={resourceType === "PERSONAL_TRAINING"} onChange={event => setSlotValues(current => ({ ...current, capacity: event.target.value }))} className="mt-2 h-11" /></label>{resourceType === "PERSONAL_TRAINING" && <label className="text-xs font-bold">المدرب<select required value={slotValues.trainerProfileId} onChange={event => setSlotValues(current => ({ ...current, trainerProfileId: event.target.value }))} className="mt-2 h-11 w-full rounded-xl border bg-background px-3 text-sm"><option value="">اختر المدرب</option>{trainers.map(trainer => <option key={itemId(trainer)} value={itemId(trainer)}>{String(trainer.displayName ?? trainer.name ?? "مدرب")}</option>)}</select></label>}<div className="flex items-end sm:col-span-2"><Button type="submit" className="w-full" disabled={savingSlot}>{savingSlot ? <Loader2 className="animate-spin" /> : <Plus />}إنشاء الموعد</Button></div></form>{slotError && <p role="alert" className="mt-4 rounded-xl bg-destructive/10 p-3 text-xs font-bold text-destructive">{slotError}</p>}<div className="mt-4">{loadingSlots ? <div className="grid place-items-center p-8"><Loader2 className="animate-spin text-primary" /></div> : slots.length ? <div className="grid gap-2 sm:grid-cols-2">{slots.map(slot => <div key={itemId(slot)} className="rounded-xl border p-3"><p className="text-xs font-bold">{new Intl.DateTimeFormat("ar-SA", { dateStyle: "medium", timeStyle: "short" }).format(new Date(String(slot.startsAt)))}</p><p className="mt-1 text-[11px] text-muted-foreground">حتى {new Intl.DateTimeFormat("ar-SA", { timeStyle: "short" }).format(new Date(String(slot.endsAt)))} · المتاح {Math.max(0, Number(slot.capacity ?? 0) - Number(slot.bookedCount ?? 0))} من {String(slot.capacity ?? 0)}</p></div>)}</div> : <p className="rounded-xl border border-dashed p-5 text-center text-xs text-muted-foreground">لا توجد مواعيد مفتوحة خلال الثلاثين يومًا القادمة.</p>}</div></section>}
...
function MasterForm({ config, mode, item, organizationId, branchId, references, setReferences, onClose, onSaved }: { config: MasterConfig; mode: "create" | "edit"; item?: RecordItem; organizationId: string; branchId: string; references: Record<string, RecordItem[]>; setReferences: React.Dispatch<React.SetStateAction<Record<string, RecordItem[]>>>; onClose: () => void; onSaved: () => void }) {
  ...
  useEffect(() => {
    ...
    const sourceKey = (source: NonNullable<Field["source"]>) => source === "facilities" || (source === "services" && config.id === "bookable-resources") ? `${source}:${branchId}` : source
    ...
    void Promise.all(missingSources.map(async source => {
      ...
      if ((source === "facilities" || source === "services" && config.id === "bookable-resources") && branchId) path.searchParams.set("branchId", branchId)
      const response = await apiRequest<unknown>(`${path.pathname}${path.search}`)
  ...
  }, [branchId, config.id, organizationId, references, requiredSources, setReferences])
  async function submit(event: React.FormEvent) {
...
const permissionSpecificNotes: Partial<Record<string, string[]>> = {
  ...
  "finance.refunds.approve": ["تتيح مراجعة طلبات الاسترداد واعتمادها أو رفضها. يمكن للموظف نفسه المراجعة إذا كان يحمل هذه الصلاحية."],
...
function MasterField({ field, value, choices, onChange }: { field: Field; value: Value | undefined; choices: RecordItem[]; onChange: (value: Value) => void }) {
  ...
  if (field.type === "textarea") { const textValue = String(value ?? ""); return <label className="text-xs font-bold sm:col-span-2"><span>{field.label}{field.required && <span className="mr-1 text-destructive">*</span>}</span><textarea required={field.required} maxLength={field.maxLength} rows={field.rows} value={textValue} onChange={event => onChange(event.target.value)} className="mt-2 min-h-24 w-full resize-y rounded-xl border bg-background p-3 text-sm leading-7 outline-none focus:border-primary focus:ring-3 focus:ring-primary/15" aria-describedby={field.hint ? `${field.name}-hint` : undefined} />{(field.hint || field.maxLength) && <span id={`${field.name}-hint`} className="mt-2 flex justify-between gap-3 text-[10px] font-normal text-muted-foreground"><span>{field.hint}</span>{field.maxLength && <span aria-live="polite" className="shrink-0 tabular-nums">{textValue.length.toLocaleString("ar-SA")} / {field.maxLength.toLocaleString("ar-SA")}</span>}</span>}</label> }
