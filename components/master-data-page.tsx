...
const configs: MasterConfig[] = [
  ...
  },
  {
    ...
    createFields: [{ name: "code", label: "رمز الباقة", required: true }, { name: "name", label: "اسم الباقة", required: true }, { name: "durationDays", label: "المدة بالأيام", type: "number", required: true }, { name: "branchAccessPolicy", label: "سياسة الوصول للفروع", type: "select", options: [{ value: "SINGLE_BRANCH", label: "فرع واحد" }, { value: "SELECTED_BRANCHES", label: "فروع مختارة" }, { value: "ALL_ORGANIZATION_BRANCHES", label: "كل الفروع" }] }, { name: "branchIds", label: "الفروع المتاحة", type: "multi", source: "branches" }, { name: "serviceIds", label: "الخدمات المشمولة", type: "multi", source: "services", required: true }, { name: "freezePolicyVersionId", label: "سياسة التجميد", type: "select", source: "policies", sourceFilter: { key: "policyType", value: "FREEZE" } }, { name: "cancellationPolicyVersionId", label: "سياسة الإلغاء", type: "select", source: "policies", sourceFilter: { key: "policyType", value: "CANCELLATION" } }, { name: "renewalPolicyVersionId", label: "سياسة التجديد", type: "select", source: "policies", sourceFilter: { key: "policyType", value: "RENEWAL" } }, { name: "visitAllowance", label: "عدد الزيارات (اختياري)", type: "number" }, { name: "description", label: "الوصف", type: "textarea" }],
    initial: { durationDays: "30", branchAccessPolicy: "SINGLE_BRANCH", fulfillmentKind: "FACILITY_ACCESS" },
    createBody: values => ({ code: values.code, name: values.name, description: values.description || undefined, durationDays: number(values.durationDays), visitAllowance: values.visitAllowance ? number(values.visitAllowance) : undefined, fulfillmentKind: "FACILITY_ACCESS", branchAccessPolicy: values.branchAccessPolicy || "SINGLE_BRANCH", branchIds: asArray(values.branchIds), entitlements: asArray(values.serviceIds).map(serviceId => ({ serviceId, visitAllowance: values.visitAllowance ? number(values.visitAllowance) : undefined })), freezePolicyVersionId: values.freezePolicyVersionId || undefined, cancellationPolicyVersionId: values.cancellationPolicyVersionId || undefined, renewalPolicyVersionId: values.renewalPolicyVersionId || undefined }),
  },
  {
  ...
  },
  {
    ...
    createFields: [{ name: "policyType", label: "نوع السياسة", type: "select", required: true, options: [{ value: "FREEZE", label: "تجميد الاشتراك" }, { value: "CANCELLATION", label: "إلغاء الاشتراك" }, { value: "RENEWAL", label: "تجديد الاشتراك" }, { value: "BOOKING_CANCELLATION", label: "إلغاء الحجز" }] }, { name: "policyKey", label: "رمز السياسة", required: true, hint: "رمز ثابت للإصدارات المتتالية، مثل STANDARD_FREEZE." }, { name: "versionNumber", label: "رقم الإصدار", type: "number", required: true }, { name: "name", label: "اسم واضح للسياسة", required: true }, { name: "maxDaysPerFreeze", label: "أقصى أيام للتجميد", type: "number", visibleWhen: { field: "policyType", values: ["FREEZE"] } }, { name: "maxFreezesPerTerm", label: "أقصى مرات تجميد للاشتراك", type: "number", visibleWhen: { field: "policyType", values: ["FREEZE"] } }, { name: "minimumActiveDaysBeforeFreeze", label: "أقل أيام نشاط قبل التجميد", type: "number", visibleWhen: { field: "policyType", values: ["FREEZE"] } }, { name: "noticeDays", label: "فترة الإشعار بالأيام", type: "number", visibleWhen: { field: "policyType", values: ["CANCELLATION"] } }, { name: "refundable", label: "السماح بالاسترداد", type: "checkbox", visibleWhen: { field: "policyType", values: ["CANCELLATION"] } }, { name: "fee", label: "رسوم الإلغاء بالريال", type: "number", visibleWhen: { field: "policyType", values: ["CANCELLATION"] } }, { name: "graceDays", label: "فترة السماح بالأيام", type: "number", visibleWhen: { field: "policyType", values: ["RENEWAL"] } }, { name: "allowEarlyRenewalDays", label: "السماح بالتجديد المبكر قبل (أيام)", type: "number", visibleWhen: { field: "policyType", values: ["RENEWAL"] } }, { name: "cutoffHours", label: "آخر موعد للإلغاء قبل الحجز (ساعات)", type: "number", visibleWhen: { field: "policyType", values: ["BOOKING_CANCELLATION"] } }, { name: "refundPercentage", label: "نسبة الاسترداد %", type: "number", visibleWhen: { field: "policyType", values: ["BOOKING_CANCELLATION"] } }],
    initial: { policyType: "FREEZE", versionNumber: "1", refundable: false },
    createBody: values => ({ policyKey: values.policyKey, versionNumber: number(values.versionNumber), policyType: values.policyType, name: values.name, configuration: values.policyType === "FREEZE" ? { maxDaysPerFreeze: number(values.maxDaysPerFreeze), maxFreezesPerTerm: number(values.maxFreezesPerTerm), minimumActiveDaysBeforeFreeze: number(values.minimumActiveDaysBeforeFreeze) } : values.policyType === "CANCELLATION" ? { noticeDays: number(values.noticeDays), refundable: Boolean(values.refundable), feeMinor: Math.round(number(values.fee) * 100) } : values.policyType === "RENEWAL" ? { graceDays: number(values.graceDays), allowEarlyRenewalDays: number(values.allowEarlyRenewalDays) } : { cutoffHours: number(values.cutoffHours), refundPercentageBps: Math.round(number(values.refundPercentage) * 100) } }),
  },
  {
  ...
  },
  {
    ...
    createFields: [{ name: "facilityId", label: "المرفق", type: "select", source: "facilities", required: true }, { name: "serviceId", label: "الخدمة المرتبطة", type: "select", source: "services", required: true }, { name: "code", label: "رمز المورد", required: true }, { name: "name", label: "اسم المورد", required: true }, { name: "type", label: "نوع الحجز", type: "select", required: true, options: [{ value: "COURT", label: "ملعب" }, { value: "CLASS", label: "حصة جماعية" }, { value: "PERSONAL_TRAINING", label: "تدريب شخصي" }] }, { name: "capacity", label: "السعة", type: "number", required: true }],
  },
  {
