...
function MasterForm({ config, mode, item, organizationId, branchId, references, setReferences, onClose, onSaved }: { config: MasterConfig; mode: "create" | "edit"; item?: RecordItem; organizationId: string; branchId: string; references: Record<string, RecordItem[]>; setReferences: React.Dispatch<React.SetStateAction<Record<string, RecordItem[]>>>; onClose: () => void; onSaved: () => void }) {
  ...
  useEffect(() => {
    ...
    const missingSources = requiredSources.filter(source => !references[source])
    if (missingSources.length === 0) return
    let cancelled = false
    const paths: Record<NonNullable<Field["source"]>, string> = { activities: "/organizations/{organizationId}/activities", categories: "/organizations/{organizationId}/service-categories", services: "/organizations/{organizationId}/services", packages: "/organizations/{organizationId}/packages", permissions: "/organizations/{organizationId}/permissions", branches: "/organizations/{organizationId}/branches", meals: "/organizations/{organizationId}/restaurant/meals" }
    void Promise.all(missingSources.map(async source => {
      const response = await apiRequest<unknown>(paths[source].replace("{organizationId}", organizationId))
      return [source, listOf(response.data)] as const
    })).then(entries => {
      if (entries.length && !cancelled) setReferences(current => ({ ...current, ...Object.fromEntries(entries) }))
    }).catch(() => undefined)
    return () => { cancelled = true }
  }, [organizationId, references, requiredSources, setReferences])
