...
const configs: MasterConfig[] = [
  ...
  },
  {
    ...
    editBody: (values, item) => ({ policyKey: item.policyKey, versionNumber: Number(item.versionNumber ?? 0) + 1, policyType: item.policyType, name: values.name, configuration: item.policyType === "FREEZE" ? { maxDaysPerFreeze: number(values.maxDaysPerFreeze), maxFreezesPerTerm: number(values.maxFreezesPerTerm), minimumActiveDaysBeforeFreeze: number(values.minimumActiveDaysBeforeFreeze) } : item.policyType === "CANCELLATION" ? { noticeDays: number(values.noticeDays), refundable: values.cancellationMode === "IMMEDIATE_PRORATED", feeMinor: Math.round(number(values.fee) * 100), cancellationMode: values.cancellationMode || "END_OF_TERM" } : item.policyType === "RENEWAL" ? { graceDays: number(values.graceDays), allowEarlyRenewalDays: number(values.allowEarlyRenewalDays) } : { cutoffHours: number(values.cutoffHours), refundPercentageBps: Math.round(number(values.refundPercentage) * 100) } }),
