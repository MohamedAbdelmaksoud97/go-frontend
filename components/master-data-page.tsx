...
const configs: MasterConfig[] = [
  ...
  },
  {
    ...
    columns: [{ label: "الرمز", key: "code" }, { label: "الاسم", key: "name" }, { label: "نوع العقد", key: "contractType" }, { label: "عنوان العقد", key: "contractTitle" }, { label: "الحالة", key: "status" }],
    createFields: [{ name: "code", label: "رمز النشاط", required: true }, { name: "name", label: "اسم النشاط", required: true }, { name: "contractType", label: "نوع نموذج العقد", type: "select", required: true, options: [{ value: "GENERAL_ACTIVITY", label: "عقد نشاط عام / بالغ" }, { value: "CHILD_ACADEMY", label: "عقد أكاديمية أطفال / ولي أمر" }], hint: "يحدد بيانات الإقرار والتوقيع التي ستظهر في النسخة المطبوعة." }, { name: "contractTitle", label: "عنوان عقد ممارسة النشاط", required: true, hint: "اكتب عنوانًا مختصرًا فقط؛ سيظهر في أعلى العقد المطبوع." }, { name: "contractContent", label: "بنود عقد ممارسة النشاط", type: "textarea", required: true, maxLength: 100000, rows: 16, hint: "أدخل البنود فقط، ويفضل أن يكون كل بند في سطر مستقل. يضيف النظام تلقائيًا بيانات النادي والنشاط والمشترك والإقرار والتوقيعات، ويحفظ نسخة ثابتة من البنود وقت البيع." }],
    editFields: [{ name: "name", label: "اسم النشاط", required: true }, { name: "contractType", label: "نوع نموذج العقد", type: "select", required: true, options: [{ value: "GENERAL_ACTIVITY", label: "عقد نشاط عام / بالغ" }, { value: "CHILD_ACADEMY", label: "عقد أكاديمية أطفال / ولي أمر" }] }, { name: "contractTitle", label: "عنوان عقد ممارسة النشاط", required: true, hint: "اكتب عنوانًا مختصرًا فقط؛ سيظهر في أعلى العقد المطبوع." }, { name: "contractContent", label: "بنود عقد ممارسة النشاط", type: "textarea", required: true, maxLength: 100000, rows: 16, hint: "أدخل البنود فقط. يضيف النظام تلقائيًا بيانات النادي والنشاط والمشترك والإقرار والتوقيعات. لن يؤثر التعديل على العقود المحفوظة سابقًا." }, { name: "status", label: "الحالة", type: "select", options: [{ value: "ACTIVE", label: "نشط" }, { value: "INACTIVE", label: "غير نشط" }] }],
  },
  {
...
function itemText(item: RecordItem, key: string, branches: BranchLookup[] = []) { if (key === "targetDisplay") { const name = String(item.targetName ?? ""); const code = String(item.targetCode ?? ""); if (name) return code && code !== name ? `${name} (${code})` : name; return item.targetType === "SERVICE" ? "خدمة غير متاحة" : "باقة غير متاحة" } if (key === "targetType") return item.targetType === "SERVICE" ? "خدمة" : item.targetType === "PACKAGE" ? "باقة" : text(item[key], key); if (key === "contractType") return item[key] === "CHILD_ACADEMY" ? "أكاديمية أطفال" : "نشاط عام"; if (key === "amountMinor" && item.amount && typeof item.amount === "object") return money((item.amount as RecordItem).minorUnits); if (key === "benefitValue" && item.benefitType === "PERCENTAGE") return `${Number(item[key]) / 100}%`; if (key === "branchId") { const branch = branches.find(candidate => candidate.id === item[key]); return branch?.nameAr ?? branch?.name ?? (item[key] ? "فرع محدد" : "جميع الفروع") } if (key === "hasLoginAccount") return item[key] ? "مفعّل" : "غير مُنشأ"; return text(item[key], key) }
