...
function FreezeHistory({ row }: { row: Row }) { const periods = Array.isArray(row.freezePeriods) ? row.freezePeriods as Row[] : []; const scheduled = pendingFreezeSchedule(row); const frozen = String(row.status ?? "").toUpperCase() === "FROZEN"; if (!periods.length && !scheduled) return null; return <div className="mt-3 rounded-xl border bg-background/60 p-3"><p className="text-xs font-black">التجميد</p>{scheduled && <p className="mt-2 rounded-lg bg-blue-500/10 p-2 text-[11px] leading-5 text-blue-700 dark:text-blue-300">مجدول ليبدأ في {formatDate(scheduled.scheduledStartAt)} لمدة {String(scheduled.requestedDays)} يوم · {String(scheduled.reason ?? "دون سبب مسجل")}</p>}{frozen && <p className="mt-2 rounded-lg bg-sky-500/10 p-2 text-[11px] font-bold leading-5 text-sky-700 dark:text-sky-300">نهاية اشتراكك المتوقعة بعد التجميد: {dateOnly(row.termEnd)}. إذا استأنفت مبكرًا سيُحتسب فقط الوقت الذي قضيته في التجميد.</p>}<div className="mt-2 space-y-1.5">{periods.map((period, index) => <p key={String(period.id ?? index)} className="text-[11px] leading-5 text-muted-foreground">{dateOnly(period.startedAt)} — {period.resumedAt ? `استؤنف في ${dateOnly(period.resumedAt)}` : `مجمّد حتى ${dateOnly(period.plannedEndAt)}`} · {String(period.reason ?? "دون سبب مسجل")}</p>)}</div></div> }
