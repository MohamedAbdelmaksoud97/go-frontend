...
type AppContextValue={grants:Grant[];organizationId:string;branchId:string;branches:Branch[];account?:AccountSummary;loading:boolean;error?:string;setOrganizationId:(id:string)=>void;setBranchId:(id:string)=>void;reload:()=>Promise<void>;canAccess:(permissions:string[])=>boolean}
...
export function AppProvider({children}:{children:React.ReactNode}){
 ...
 const value=useMemo<AppContextValue>(()=>({grants,organizationId,branchId,branches,account,loading,error,setOrganizationId,setBranchId,reload,canAccess:permissions=>permissions.length===0||!hasRuntimeApi()||(!loading&&permissions.some(permission=>can(grants,permission,organizationId,branchId)))}),[grants,organizationId,branchId,branches,account,loading,error,setOrganizationId,setBranchId,reload])
