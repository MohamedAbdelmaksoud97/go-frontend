...
type AppContextValue={userAccountId:string;grants:Grant[];organizationId:string;branchId:string;branches:Branch[];account?:AccountSummary;loading:boolean;error?:string;setOrganizationId:(id:string)=>void;setBranchId:(id:string)=>void;reload:()=>Promise<void>;canAccess:(permissions:string[])=>boolean}
const AppContext=createContext<AppContextValue|undefined>(undefined)

function stored(key:string){if(typeof window==="undefined")return "";return localStorage.getItem(key)??""}
export function AppProvider({children}:{children:React.ReactNode}){
 const [userAccountId,setUserAccountId]=useState("");const [grants,setGrants]=useState<Grant[]>([]);const [organizationId,setOrganizationState]=useState("");const [branchId,setBranchState]=useState("");const [branches,setBranches]=useState<Branch[]>([]);const [loading,setLoading]=useState(hasRuntimeApi());const [error,setError]=useState<string>()
 ...
 const value=useMemo<AppContextValue>(()=>({userAccountId,grants,organizationId,branchId,branches,account,loading,error,setOrganizationId,setBranchId,reload,canAccess:permissions=>permissions.length===0||!hasRuntimeApi()||(!loading&&permissions.some(permission=>can(grants,permission,organizationId,branchId)))}),[userAccountId,grants,organizationId,branchId,branches,account,loading,error,setOrganizationId,setBranchId,reload])
