...
export function SystemUserGuide({ pages, page }: { pages: GuidePage[]; page?: GuidePage }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)]">
    ...
    <div className="min-w-0 space-y-5">
      ...
      <nav className="flex max-w-full snap-x snap-mandatory gap-2 overflow-x-auto overscroll-x-contain py-1" aria-label="عناوين هذه الصفحة">{page.sections.map(section => <a key={section.id} href={`#${section.id}`} className="shrink-0 snap-start whitespace-nowrap rounded-full border bg-card px-3 py-2 text-[11px] font-bold transition hover:border-primary hover:text-amber-700">{section.title}</a>)}</nav>
