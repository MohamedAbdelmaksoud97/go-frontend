...
type MemberRow = Row & {
  memberId?: string; memberName?: string; memberNumber?: string; nationalId?: string; phoneE164?: string; branchId?: string; branchName?: string
...
export function TrainerWorkspace() {
  ...
  const canRecordMeasurements = context.canAccess(["measurements.manage"])
  const canManagePlans = context.canAccess(["coaching.training-plans.manage"])
  ...
  const filteredMembers = data.members.filter(member => normalize(`${member.memberName} ${member.memberNumber} ${member.phoneE164} ${member.nationalId}`).includes(normalize(query)))
  ...
  return <div className="fade-up space-y-6">
    <section className="overflow-hidden rounded-[2rem] border border-primary/20 bg-gradient-to-l from-primary/[.12] via-card to-card p-6 sm:p-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center">
        ...
        <div className="flex flex-wrap gap-2 lg:mr-auto">
          {canRecordMeasurements && <Button onClick={() => setMeasurementMemberId(selectedMemberId || data.members[0]?.memberId || "")} disabled={!data.members.length}><Ruler />تسجيل قياس</Button>}
          {canManagePlans && <Button variant="outline" onClick={() => setPlanMemberId(selectedMemberId || data.members[0]?.memberId || "")} disabled={!data.members.length}><Plus />خطة تدريب جديدة</Button>}
    ...
    {loading ? <div className="grid min-h-80 place-items-center rounded-3xl border bg-card"><div className="text-center"><Loader2 className="mx-auto animate-spin text-primary" /><p className="mt-3 text-sm text-muted-foreground">نجهّز مساحة عملك…</p></div></div> : <>
      ...
      {active === "members" && <MembersView members={filteredMembers} query={query} setQuery={setQuery} selected={selectedMember} select={setSelectedMemberId} measurements={measurements} measurementsLoading={measurementsLoading} onMeasurement={setMeasurementMemberId} onPlan={setPlanMemberId} canRecordMeasurements={canRecordMeasurements} canManagePlans={canManagePlans} />}
...
function MembersView({ members, query, setQuery, selected, select, measurements, measurementsLoading, onMeasurement, onPlan, canRecordMeasurements, canManagePlans }: { members: MemberRow[]; query: string; setQuery: (value: string) => void; selected?: MemberRow; select: (id: string) => void; measurements: Row[]; measurementsLoading: boolean; onMeasurement: (id: string) => void; onPlan: (id: string) => void; canRecordMeasurements: boolean; canManagePlans: boolean }) {
  return <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
    <Card><CardContent className="p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center"><div><h2 className="font-black">المتدربون المرتبطون بك</h2><p className="mt-1 text-xs text-muted-foreground">لا يظهر هنا إلا أعضاء التعيينات النشطة داخل فروع عملك.</p></div><label className="relative sm:mr-auto sm:w-80"><Search className="absolute right-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="الاسم، العضوية، الجوال أو الهوية" className="pr-10" /></label></div><div className="mt-5 grid gap-3 md:grid-cols-2">{members.map(member => { const progress = percentage(member.planCompletedItems, member.planTotalItems); return <button key={member.memberId} onClick={() => select(String(member.memberId))} className={`rounded-3xl border p-4 text-right transition ${selected?.memberId === member.memberId ? "border-primary bg-primary/[.07] shadow-lg shadow-primary/5" : "hover:border-primary/35 hover:bg-secondary/30"}`}><div className="flex items-center gap-3"><Avatar name={member.memberName} /><div className="min-w-0"><p className="truncate font-black">{member.memberName}</p><p className="mt-1 text-[11px] text-muted-foreground">{member.memberNumber} · {member.branchName}</p>{member.phoneE164 && <p className="mt-1 text-[11px] text-muted-foreground">{member.phoneE164}</p>}</div><Badge variant="outline" className="mr-auto">{statusLabel(member.status)}</Badge></div><div className="mt-4"><div className="flex justify-between text-[11px]"><span className="font-bold">{member.activePlanName ?? "لا توجد خطة نشطة"}</span><span className="text-muted-foreground">{member.activePlanId ? `${progress}%` : "—"}</span></div><Progress value={progress} /></div>{member.nextSessionAt && <p className="mt-3 flex items-center gap-1.5 text-[11px] text-muted-foreground"><CalendarDays className="size-3.5 text-primary" />الجلسة القادمة: {dateTime(member.nextSessionAt)}</p>}</button>})}{!members.length && <div className="md:col-span-2"><Empty icon={Users} title="لا توجد نتائج" description={query ? "جرّب الاسم أو العضوية أو الجوال أو رقم الهوية." : "لم يتم تعيين أعضاء لهذا المدرب بعد."} /></div>}</div></CardContent></Card>
    <MemberProfile member={selected} measurements={measurements} loading={measurementsLoading} onMeasurement={onMeasurement} onPlan={onPlan} canRecordMeasurements={canRecordMeasurements} canManagePlans={canManagePlans} />
  </div>
}

function MemberProfile({ member, measurements, loading, onMeasurement, onPlan, canRecordMeasurements, canManagePlans }: { member?: MemberRow; measurements: Row[]; loading: boolean; onMeasurement: (id: string) => void; onPlan: (id: string) => void; canRecordMeasurements: boolean; canManagePlans: boolean }) {
