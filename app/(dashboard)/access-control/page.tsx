...
export default function AccessControlPage(){
 ...
 return <div className="space-y-6 fade-up">
  <header className="flex flex-wrap items-start justify-between gap-4"><div><Badge variant="outline"><RadioTower/>التحكم في الوصول</Badge><h1 className="mt-4 text-3xl font-black">البوابات والبصمة</h1><p className="mt-2 max-w-3xl text-sm leading-7 text-muted-foreground">ربط لوحة ZKTeco بقرارات الدخول في GO ومتابعة كل حدث دون تخزين قوالب البصمة.</p></div><div className="flex gap-2"><Button variant="outline" onClick={()=>void load()} disabled={loading}><RefreshCw className={loading?"animate-spin":""}/>تحديث</Button>{canManage&&<Button onClick={()=>setRegisterOpen(true)}><RadioTower/>تسجيل لوحة</Button>}</div></header>
