...
import { accessDeviceReason, accessReader, accessSeverity, accessSubject, accessSubjectNumber, accessSystemReason, gateOpenedButSystemRejected, type GateAccessEvent } from "@/lib/access-event"
...
type AccessEvent=GateAccessEvent
...
type Employee={id:string;name?:string;employeeNumber:string;phoneE164?:string;status?:string}
type Credential={id:string;memberId?:string;employeeId?:string;credentialType:string;credentialValue:string;status:"ACTIVE"|"REVOKED";issuedAt?:string}
...
export default function AccessControlPage(){
 ...
 const accepted=currentEvents.filter(event=>event.processingCode==="SYSTEM_ACCEPTED"||event.processingCode?.startsWith("EMPLOYEE_CLOCK_")).length
 const unresolved=currentEvents.filter(event=>event.processingStatus==="UNMAPPED_CREDENTIAL"||event.processingStatus==="FAILED"||event.processingCode?.startsWith("SYSTEM_REJECTED_")).length
 ...
 return <div className="space-y-6 fade-up">
  ...
  <Card><CardHeader><div><CardTitle>سجل أحداث البوابة</CardTitle><CardDescription>السجل الخام محفوظ للمتابعة، وقرار GO ظاهر بجوار قرار اللوحة. الصف الأحمر يعني أن اللوحة فتحت لكن النظام رفض تسجيل الدخول.</CardDescription></div></CardHeader><CardContent className="overflow-x-auto p-0"><table className="w-full min-w-[900px] text-sm"><thead className="border-y bg-secondary/50 text-xs text-muted-foreground"><tr><th className="p-3 text-right">الوقت</th><th className="p-3 text-right">العضو / الموظف</th><th className="p-3 text-right">PIN</th><th className="p-3 text-right">نقطة القراءة</th><th className="p-3 text-right">قرار اللوحة</th><th className="p-3 text-right">معالجة GO</th></tr></thead><tbody>{loading?<tr><td colSpan={6}><Loading/></td></tr>:currentEvents.length===0?<tr><td colSpan={6}><Empty text="لم تصل أحداث من الـBridge لهذا الفرع بعد."/></td></tr>:currentEvents.map(event=><tr key={event.id} className={`border-b last:border-0 ${gateOpenedButSystemRejected(event)?"bg-red-500/10":""}`}><td className="p-3 whitespace-nowrap">{formatDate(event.deviceOccurredAt)}</td><td className="p-3"><span className="font-bold">{accessSubject(event)}</span>{accessSubjectNumber(event)&&<span className="mt-1 block text-xs text-muted-foreground">{accessSubjectNumber(event)}</span>}</td><td className="p-3 font-mono" dir="ltr">{event.credentialPin??"—"}</td><td className="p-3"><span className="font-bold">{accessReader(event)}</span><span className="mt-1 block text-xs text-muted-foreground">منفذ اللوحة {event.doorNumber}</span></td><td className="p-3"><Badge variant={event.deviceDecision==="ALLOWED"?"success":event.deviceDecision==="DENIED"?"danger":"outline"}>{event.deviceDecision==="ALLOWED"?"فتح":event.deviceDecision==="DENIED"?accessDeviceReason(event):"غير معروف"}</Badge></td><td className="p-3"><Badge variant={accessSeverity(event)}>{accessSystemReason(event)}</Badge></td></tr>)}</tbody></table></CardContent></Card>
...
function PinAssignment({canManage,organizationId}:{canManage:boolean;organizationId:string}){
 const context=useAppContext();const [subjectType,setSubjectType]=useState<"MEMBER"|"EMPLOYEE">("MEMBER");const [query,setQuery]=useState("");const [people,setPeople]=useState<Array<Member|Employee>>([]);const [selected,setSelected]=useState<Member|Employee>();const [pin,setPin]=useState("");const [current,setCurrent]=useState<Credential>();const [busy,setBusy]=useState(false);const [message,setMessage]=useState("");const [error,setError]=useState("")
 useEffect(()=>{if(!selected&&query.trim().length>=2){const timer=window.setTimeout(()=>{const path=subjectType==="MEMBER"?`/organizations/${organizationId}/members?branchId=${context.branchId}&search=${encodeURIComponent(query.trim())}&limit=12`:`/organizations/${organizationId}/employees?branchId=${context.branchId}&search=${encodeURIComponent(query.trim())}&limit=12`;apiRequest<Array<Member|Employee>|{items:Array<Member|Employee>}>(path).then(response=>setPeople(Array.isArray(response.data)?response.data:response.data.items??[])).catch(()=>setPeople([]))},250);return()=>window.clearTimeout(timer)}},[context.branchId,organizationId,query,selected,subjectType])
 async function choose(person:Member|Employee){setSelected(person);setQuery(personName(person));setPeople([]);setCurrent(undefined);setPin("");setError("");try{const response=await apiRequest<Credential[]>(`/organizations/${organizationId}/access-credentials?subjectType=${subjectType}&subjectId=${person.id}`);const credential=response.data.find(item=>item.credentialType==="FINGERPRINT_USER_ID"&&item.status==="ACTIVE");setCurrent(credential);setPin(credential?.credentialValue??"")}catch(reason){setError(humanError(reason,"تعذر تحميل رقم البصمة. حاول مجددًا."))}}
 async function assign(){if(!selected)return;setBusy(true);setError("");setMessage("");try{const response=await apiRequest<Credential>(`/organizations/${organizationId}/access-credentials/fingerprint-pins`,{method:"POST",body:JSON.stringify({subjectType,subjectId:selected.id,pin})});setCurrent(response.data);setMessage(`تم ربط PIN ${pin} بـ ${personName(selected)}.`)}catch(reason){setError(humanError(reason,"تعذر ربط رقم البصمة. تأكد أنه أرقام فقط وغير مستخدم لشخص آخر."))}finally{setBusy(false)}}
 function switchType(value:"MEMBER"|"EMPLOYEE"){setSubjectType(value);setQuery("");setSelected(undefined);setPeople([]);setPin("");setCurrent(undefined);setMessage("");setError("")}
 return <Card><CardHeader><div><CardTitle>ربط PIN البصمة</CardTitle><CardDescription>اربط الرقم الرقمي بعضو أو موظف. يظل رقم العضوية أو الموظف كما هو ولا يُرسل للنظام قالب البصمة نفسه.</CardDescription></div></CardHeader><CardContent><div className="mb-4 flex gap-2"><Button type="button" size="sm" variant={subjectType==="MEMBER"?"default":"outline"} onClick={()=>switchType("MEMBER")}>عضو</Button><Button type="button" size="sm" variant={subjectType==="EMPLOYEE"?"default":"outline"} onClick={()=>switchType("EMPLOYEE")}>موظف</Button></div><div className="grid gap-4 md:grid-cols-[1fr_.55fr_auto]"><label className="relative text-xs font-bold">{subjectType==="MEMBER"?"العضو":"الموظف"}<div className="relative"><Search className="pointer-events-none absolute right-3 top-1/2 z-10 size-4 -translate-y-1/2 text-muted-foreground"/><input className={`${fieldClass} pr-10`} value={query} onChange={event=>{setSelected(undefined);setCurrent(undefined);setPeople([]);setQuery(event.target.value)}} placeholder={subjectType==="MEMBER"?"الاسم أو رقم العضوية أو الجوال":"اسم الموظف أو رقمه أو جواله"} autoComplete="off"/>{people.length>0&&<div className="absolute inset-x-0 top-full z-30 mt-2 max-h-64 overflow-auto rounded-xl border bg-popover p-2 shadow-xl">{people.map(person=><button key={person.id} type="button" onClick={()=>void choose(person)} className="block w-full rounded-lg p-3 text-right hover:bg-secondary"><strong>{personName(person)}</strong><span className="mt-1 block text-xs text-muted-foreground" dir="ltr">{personNumber(person)??"—"}</span></button>)}</div>}</div></label><label className="text-xs font-bold">PIN جهاز البصمة<input className={fieldClass} dir="ltr" inputMode="numeric" pattern="[1-9][0-9]{0,8}" maxLength={9} value={pin} onChange={event=>setPin(event.target.value.replace(/\D/gu,"").replace(/^0+/u,"").slice(0,9))} placeholder="مثال: 22782"/></label><Button className="mt-auto h-11" disabled={!canManage||!selected||!pin||busy} onClick={()=>void assign()}>{busy?<Loader2 className="animate-spin"/>:<UserRoundCog/>}{current?"حفظ التصحيح":"ربط PIN"}</Button></div>{current&&<p className="mt-3 text-xs text-muted-foreground">PIN المرتبط حاليًا: <strong dir="ltr">{current.credentialValue}</strong></p>}{message&&<p role="status" className="mt-3 text-sm font-bold text-emerald-600">{message}</p>}{error&&<p role="alert" className="mt-3 text-sm font-bold text-red-600">{error}</p>}</CardContent></Card>
}
...
function personName(person:Member|Employee){return "employeeNumber" in person?(person.name??"موظف بدون اسم"):memberName(person)}
function personNumber(person:Member|Employee){return "employeeNumber" in person?person.employeeNumber:person.memberNumber}
