  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (staff || memberUsesEmail) {
        if (!/^\S+@\S+\.\S+$/u.test(email.trim())) throw new Error("أدخل بريدًا إلكترونيًا صحيحًا.")
        if (hasRuntimeApi()) await signInWithPassword(staff ? "staff" : "member-test", { email: email.trim(), password })
      } else {
